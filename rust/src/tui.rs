...
fn cfg_schema() -> Vec<(&'static str, &'static str, CfgKind, bool)> {
    ...
    vec![
        ...
        ("upgrade_glyphs2", "upgrade .glyphs v2→v3 (builder3)", CfgKind::Bool, true),
...
fn cfg_group(key: &str) -> &'static str {
    match key {
        ...
        "backend" | "orchestrator" | "python_policy" | "upgrade_glyphs2" | "fontc_bin" | "auto_provision" | "manage_venvs" | "jobs" | "timeout" => "Build engine",
...
const TUI_PERSIST: &[&str] = &[
    "source", "backend", "orchestrator", "python_policy", "upgrade_glyphs2", "jobs", "percent", "compare", "manage_venvs", "fontspector_qa", "build_debs",
];
...
fn cfg_apply_live(fields: &[CfgField], snap: &Snapshot, src: &dyn Source) -> String {
    ...
    if changed("upgrade_glyphs2") {
        if let Some(u) = new.get("upgrade_glyphs2").and_then(|v| v.as_bool()) {
            set.upgrade_glyphs2 = Some(u);
        }
    }
    if changed("jobs") {
...
fn field_help(key: &str) -> &str {
    match key {
        ...
        "upgrade_glyphs2" => "pre-upgrade format-2 .glyphs sources to format 3 before a builder3 build (works around babelfont-rs WrongConvertor); off = build sources as-is",
